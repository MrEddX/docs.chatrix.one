...
Възможно е да попадате в някое от долу изброените ограничения:

- Към момента, регистрацията през онлайн форма или от инсталиран клиент НЕ е със [статус](https://docs.chatrix.one/status/#_2) ОТВОРЕНА;
- Допуска се само една регистрация в рамките на един час от един и същ **IP** адрес;
